...
int main() {
	...
	snprintf(buffer, 256, "./records/%s", userInformation.userName);
	mkdir(buffer, 0777);
	...
	strcpy(chatingListWidgetParams->userName, userInformation.userName);
	...
	bindCDKObject(vMENTRY, main_page.inputWidget, KEY_F8, jumpToChatingListWidget, chatingListWidgetParams);
	...
	strcpy(requestListWidgetParams->userName, userInformation.userName);
	...
	bindCDKObject(vMENTRY, main_page.inputWidget, KEY_F7, jumpToRequestListWidget, requestListWidgetParams);
