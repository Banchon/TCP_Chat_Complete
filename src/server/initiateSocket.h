...
int initiateServerListener(int nPort, int nLengthOfQueueOfListen, char *strBoundIP) {
	...
	listenerAddr.sin_family = AF_INET;
	listenerAddr.sin_port = htons(nPort);

	if(strBoundIP == NULL)
		listenerAddr.sin_addr.s_addr = htonl(INADDR_ANY);
	else {
			if(::inet_pton(AF_INET, strBoundIP, &listenerAddr.sin_addr.s_addr) != 1) {
...
//	printf("IP %d PORT %d\n", ntohl(listenerAddr.sin_addr.s_addr), ntohs(listenerAddr.sin_port));
